...
class UserModel
{
     ...
     */
    public function update($name, $lname, $gender, $id)
    {
        $isSuccess = false;
        try{
            $sql = "UPDATE user_table SET `first_name`=?,`last_name`=?, `gender`=? WHERE id=?";
            $stmt = $this->dbCon->prepare($sql);
            $update = $stmt->execute([$name, $lname, $gender, $id]);
            if($update){
    ...
    /**
     * This will fetch the user by username and password
     * empty row meaning no match
     */
    public function getuserByCredentials($username, $password){
        $rows = [];
        try{
            $sql = "SELECT * FROM login_users WHERE username = ? AND password = ? LIMIT 1" ;
            $stmt = $this->dbCon->prepare($sql);
    ...
    public function getUserByUsername($username){
        $rows = [];
        try{
            $sql = "SELECT * FROM login_users WHERE username = ? LIMIT 1" ;
            $stmt = $this->dbCon->prepare($sql);
